...
### ******* WORKED W/ KEVIN LI ON ALL OF THIS ********* ####
...
class PolicyGradient(object):
    ...
    def discount_rewards(self, r, do_reset=False):
        ...
        for t in reversed(range(0, r.size)): 
            if do_reset and r[t] != 0:
                cumsum = 0 
            cumsum = cumsum * self.gamma + r[t]
            discounted_r[t] = cumsum 
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
    ...
    def policy_backward(self, ep_x, ep_h, ep_dprobs, discounted_epr):
        ...
        ########################################################################
        score = ep_dprobs * discounted_epr
        dW2 = np.dot(ep_h.T, score).ravel()
        dh = np.outer(score, self.model['W2'])
        dh[ep_h <= 0] = 0 
